...
if (!nexacro.ChartPyramidSeriesControl) {
	...
	_pChartPyramidSeriesControl.set_graphratio = function (val) {
		this._set_margine("graphratio", val);
	};
	_pChartPyramidSeriesControl.set_marginleftright = function (val) {
		this._set_margine("marginleftright", val);
	};
	_pChartPyramidSeriesControl.set_margintopdown = function (val) {
		this._set_margine("margintopdown", val);
	};

	_pChartPyramidSeriesControl._set_margine = function (type, val) {
		var lVal = null;
		...
		if (this[type] != val) {
			this._changeContentsProperty(type, val, this[type]);
			this[type] = val;
			this["on_apply_" + type](lVal);
		}
